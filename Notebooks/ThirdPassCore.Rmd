...
# ASV Level patterns over all days
...
```
There is not interaction between time and outcome on community.
...
```{r}
ggsave(here("Figures", "CCA_Days3-12.png"), plot = plt2, height = 4, width = 8)
```



# Testing for differences between groups during each day. ASV only
...
# Patterns at each taxonomic level, at each day.
...
Anova all of these and make into one table.
...
# Redo CCA plot and analysis with family level patterns.
...
Statistical analysis of family level patterns
...
Extracting the family level patterns for plotting
```{r}
metawhole <- metabeast %>%
  mutate(cca = map(.x = comm, ~cca(sqrt(.x) ~ Info1 + DayN, data = metadata_main)))
...
## Updated figure
Used in proposal and figure. Compares good and crash at the family level.
...
```{r}
ggsave(here("Figures", "Pre_Prop_Fig1.png"), pre_prop_fig1, height = 5, width = 8)
ggsave(here("Figures", "Fig2_FamilyLevelCCA.jpeg"), pre_prop_fig1, height = 5, width = 8)
```
