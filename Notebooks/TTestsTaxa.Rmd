...
First run ThirdPassCore.Rmd
Then Run Follow_Taxa_Info1.Rmd
```{r}
...
ggplot(aes(x = as.numeric(DayN), y = log10(ratio + 1/1000), shape = Info1, color = Info1, group = `Brood#`)) +
  ...
  theme(legend.position = c(0.9, 0.6),
        legend.box.background = element_rect(linewidth = .4),
        legend.box.margin = margin(1,7,1,1))
plot_All_35
```

## As above but including Dinophyceae

```{r}
plot_harbingers <- plot_grid(
  plot_All_35,
  plot_Dinophyceae, 
  ncol = 1,
  rel_heights = c(1.25,2)
)
ggsave(here("Figures", "Fig5_Harbingers.jpg"),plot = plot_harbingers, width = 8, height = 6)
```
